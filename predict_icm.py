...
def estimate_label_icm(
    ...
    with open(os.path.join(cfg.out_param_dir, cfg.label_param_filename), 'r') as f:
        label_param = json.load(f)
    with open(os.path.join(cfg.out_param_dir, cfg.pixel_param_filename), 'r') as f:
    ...
    if region_output_dir:
        save_region_growing_image_from_regions(H, get_regions_from_connections(connections), os.path.join(region_output_dir, f"{image_stem}_0000.png"))
    ...
    for iteration in range(num_iterations):
        ...
        print(f"    [OK] Iteration completed in {time.time() - iter_start_time:.2f}s  (Nodes changed: {num_changed}/{len(leaf_nodes)})")
        
        if region_output_dir:
            save_region_growing_image_from_regions(H, current_regions, os.path.join(region_output_dir, f"{image_stem}_{iteration + 1:04d}.png"))
        ...
        if true_label_array is not None:
            ...
            if label_diff_output_dir:
                save_diff_image(X_est, true_label_array, os.path.join(label_diff_output_dir, f"{image_stem}_{iteration + 1:04d}.png"))
        ...
        if num_changed == 0:
            ...
            print(f"    [OK] Converged at iteration {iteration+1} (no connections changed)")
            break
            
    if X_est is None:
        X_est = _estimate_labels_from_regions(get_regions_from_connections(connections), H, W, image, label_param_with_size, pixel_param, node_likelihood_cache)
...
def estimate_segmentation(image_path, cfg: Config, oa_error_csv_path=None):
    ...
    return estimate_label_icm(
        ...
        quadtree_output_path=quadtree_output_path, image_stem=image_stem,
        label_output_dir=label_output_dir, label_vis_output_dir=label_vis_output_dir,
        label_diff_output_dir=label_diff_output_dir, oa_log_filepath=image_oa_log_path,
        true_label_array=true_label_array, label_to_value_map=label_to_value_map,
...
def process_test_images(cfg: Config, cfg_module) -> None:
    """
    すべてのテスト画像に対してセグメンテーション推定を一括実行する (ICM)
    """
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="ddCRP Image Segmentation (ICM)")
