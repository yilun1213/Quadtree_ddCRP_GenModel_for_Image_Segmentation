...
def _theta() -> dict:
    ...
    # ----- x=0: smooth isotropic -----
    # 4x4 causal neighborhood (15 offsets), A_Δ = s(d(Δ))·I_3
    # Chebyshev d=1: s=0.15 (det=3.375e-3)
    # Chebyshev d=2: s=0.06 (det=2.160e-4)
    # Chebyshev d=3: s=0.02 (det=8.000e-6)
    x0_ar = {
        "(0,-1)":   sI(0.15), "(-1,0)":  sI(0.15), "(-1,-1)": sI(0.15),
        "(0,-2)":   sI(0.06), "(-1,-2)": sI(0.06), "(-2,0)":  sI(0.06),
        "(-2,-1)":  sI(0.06), "(-2,-2)": sI(0.06),
        "(0,-3)":   sI(0.02), "(-1,-3)": sI(0.02), "(-2,-3)": sI(0.02),
        "(-3,0)":   sI(0.02), "(-3,-1)": sI(0.02), "(-3,-2)": sI(0.02),
        "(-3,-3)":  sI(0.02),
    }

    # ----- x=1: horizontal banding -----
    # Strong positive AR in horizontal direction (Δj < 0), weak vertical
    # (0,-1): diag(0.40, 0.36, 0.32)  det=4.608e-2
    # (0,-2): diag(0.18, 0.16, 0.14)  det=4.032e-3
    # (0,-3): diag(0.07, 0.06, 0.05)  det=2.100e-4
    # (-1,0): 0.03·I_3                det=2.700e-5
    x1_ar = {
        "(0,-1)": d3(0.40, 0.36, 0.32),
        "(0,-2)": d3(0.18, 0.16, 0.14),
        "(0,-3)": d3(0.07, 0.06, 0.05),
        "(-1,0)": sI(0.03),
    }

    # ----- x=2: diagonal oscillation -----
    # Alternating sign along main diagonal (creates diagonal checker-like pattern)
    # (-1,-1): diag(0.28, 0.26, 0.24)    det= 1.747e-2
    # (-2,-2): diag(-0.12,-0.11,-0.10)   det=-1.320e-3
    # (-3,-3): diag(0.04, 0.04, 0.03)    det= 4.800e-5
    # (-1,0) and (0,-1): 0.06·I_3        det= 2.160e-4
    x2_ar = {
        "(-1,-1)": d3(0.28, 0.26, 0.24),
        "(-2,-2)": d3(-0.12, -0.11, -0.10),
        "(-3,-3)": d3(0.04, 0.04, 0.03),
        "(-1,0)":  sI(0.06),
        "(0,-1)":  sI(0.06),
    }
