...
def sample_connection(leaf_node, connections, leaf_nodes, adjacency_dict, 
    ...
    for candidate in candidates:
        ...
        score = prior_prob + log_likelihood_ratio
        # 無効値は候補から実質除外する（後段で安全に正規化）
        if np.isnan(score):
            score = -np.inf
        log_probs.append(float(score))
...
def estimate_label_gibbs_sampling(
    ...
    print(f"  Configuration: num_iterations={num_iterations}, alpha={alpha}")
