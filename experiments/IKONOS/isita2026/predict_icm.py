...
def estimate_segmentation(image_path, cfg: Config, oa_error_csv_path=None):
    ...
    # Config is frozen; keep it immutable and derive ICM-specific output root locally.
    icm_output_root = cfg.est_label_folder_path.replace("estimation_results", "estimation_results_icm")

    region_output_dir = os.path.join(icm_output_root, cfg.est_region_dirname)
    quadtree_output_dir = os.path.join(icm_output_root, cfg.est_quadtree_dirname)
    label_output_dir = os.path.join(icm_output_root, cfg.est_label_dirname)
    ...
    if oa_error_csv_path is None:
        default_oa_csv_path = getattr(cfg, 'oa_error_csv_path', None)
        if default_oa_csv_path is not None:
            oa_error_csv_path = default_oa_csv_path.replace("estimation_results", "estimation_results_icm", 1)
